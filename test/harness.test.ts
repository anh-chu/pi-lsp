...
test('pi-test-harness baseline session does not load pi-lsp tools', async () => {
  ...
  try {
    ...
    assert.equal(toolNames.includes('pi_lsp_get_symbol'), false);
    assert.equal(toolNames.includes('pi_lsp_find_definition'), false);
...
test('pi-test-harness treatment session loads pi-lsp and exercises symbol lookup tool path', async () => {
  ...
  try {
    ...
    const toolNames = session.session.agent.state.tools.map((tool: { name: string }) => tool.name);
    assert.equal(toolNames.includes('pi_lsp_get_symbol'), true);
    assert.equal(toolNames.includes('pi_lsp_find_definition'), true);
    assert.equal(toolNames.includes('pi_lsp_find_references'), true);
    assert.equal(toolNames.includes('pi_lsp_rank_context'), true);

    await session.run(
      when('Show exact hello definition', [
        calls('pi_lsp_get_symbol', { symbol: 'hello', file: 'src/demo.ts', contextLines: 0 }),
        says('Definition captured.'),
      ]),
    );

    const symbolResults = session.events.toolResultsFor('pi_lsp_get_symbol');
    ...
    assert.equal(session.events.toolCallsFor('pi_lsp_get_symbol').length, 1);
  } finally {
    disposeSession(session);
    removeDir(cwd);
    resetState();
  ...
  try {
    ...
    await session.run(
      when('Inspect readRoutes symbol first', [
        calls('pi_lsp_get_symbol', { symbol: 'readRoutes', file: 'src/routes.ts', contextLines: 0 }),
        says('Captured symbol.'),
      ]),
      when('Rank next context for route parsing', [
        calls('pi_lsp_rank_context', { query: 'route parsing', limit: 5 }),
        says('Ranked context returned.'),
      ]),
    );

    const rankResults = session.events.toolResultsFor('pi_lsp_rank_context');
